...
const { verifyAccessToUpdateComment, verifyAccessToDeleteComment, verifyAccessToLikeAndComment,verifyAccessToUpdateReport ,verifyAccessToDeleteReport} = require('../../middlewares/post-verification');
...
//-------------------------------------------------------------------
router.post('/report/:post_id',verifyAccessToLikeAndComment, async (req,res)=>{
    await DB_post.addreport(req.params.post_id, req.user.USER_ID, req.body.body);
    if(req.body.type === 'group_post')
        res.redirect(`/api/${req.body.type}/${req.body.group_id}/${req.params.post_id}`);
    else
        res.redirect(`/api/${req.body.type}/${req.params.post_id}`);
...
router.post('/report/update/:report_id', verifyAccessToUpdateReport, async (req, res) => {
    await DB_post.updatereportt(req.params.report_id, req.body.body);
    if(req.body.type === 'group_post')
        res.redirect(`/api/${req.body.type}/${req.body.group_id}/${req.body.post_id}`);
    else
        res.redirect(`/api/${req.body.type}/${req.body.post_id}`);
...
router.post('/report/delete/:report_id', verifyAccessToDeleteReport , async(req,res)=>{
    await DB_post.deletereport(req.params.report_id);
    if(req.body.type === 'group_post')
        res.redirect(`/api/${req.body.type}/${req.body.group_id}/${req.body.post_id}`);
    else
        res.redirect(`/api/${req.body.type}/${req.body.post_id}`);
...
router.get('/report/:post_id', verifyAccessToLikeAndComment, async(req,res)=>{
    res.send(await DB_post.getreports(req.params.post_id));
})
//------------------------------------------------------------------------------------------------
