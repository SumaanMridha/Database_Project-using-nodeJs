...
async function verifyAccessToUpdateReport(req,res,next){
    const cookie  = req.header('cookie');
    if(!cookie) return res.redirect('/api/auth/login?status=Access Denied');
    const token = cookie.slice(11);
    try{
...
async function verifyAccessToDeleteReport(req,res,next){
    const cookie  = req.header('cookie');
    if(!cookie) return res.redirect('/api/auth/login?status=Access Denied');
    const token = cookie.slice(11);
    try{
...
module.exports = {
    ...
    verifyAccessToUpdateReport,
    verifyAccessToDeleteReport
}